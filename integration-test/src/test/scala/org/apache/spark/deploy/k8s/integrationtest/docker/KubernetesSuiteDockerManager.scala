...
import com.spotify.docker.client.{DefaultDockerClient, DockerCertificates, DockerClient, LoggingBuildHandler}
  ...
  def buildSparkDockerImages(): Unit = {
    if (userProvidedDockerImageTag.isEmpty) {
      ...
      dockerClient.build(
        ...
        s"spark-examples-file-server:$resolvedDockerImageTag",
